...
int parse_response(char buf[], char *body, size_t slen)
{
    ...
    const char *msg;
    size_t msg_len;
    struct phr_header parsed_headers[6];
    struct phr_header response_body[6];
    size_t num_headers;
    size_t num_response_elements;
...
int process_msg3(sgx_ra_msg1_t *msg1, sgx_ra_msg3_t **msg3, size_t msg3_size, ra_msg4_t *msg4)
{
    ...
    sgx_rijndael128_cmac_msg(&kdk, (unsigned char *)("\x01VK\x00\x80\x00"), 6, &vk);

    /* Build our plaintext */

    memcpy(msg_rdata, &(*msg3)->g_a, 64);
    memcpy(&msg_rdata[64], &p_public, 64);
    memcpy(&msg_rdata[128], vk, 16);

    /* SHA-256 hash */

    sgx_sha256_msg(msg_rdata, 144, &vfy_rdata);

    mbedtls_printf("Enclave Report Verification\n");

    mbedtls_printf("VK                 = %s\n",
                   hexstring(vk, 16));
    mbedtls_printf("SHA256(Ga||Gb||VK) = %s\n",
                   hexstring(vfy_rdata, 32));
    mbedtls_printf("report_data[64]    = %s\n",
                   hexstring(&r->report_data, 64));

    if (memcmp((void *)vfy_rdata, (void *)&r->report_data, 32) != 0)
    {
        mbedtls_printf("Report verification failed.\n");
        free(b64quote);
        free(msg3);
        return 0;
    }
